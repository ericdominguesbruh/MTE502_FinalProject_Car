//V5: use accelerometer to stop at bottom of ramp
#include "MazeFunc.h"
#include <avr/io.h>
...
const int maze_speed = 120;
const int turn_speed = 70;

const int ramp_angle = 2; //ramp angle threshhold
const int flat_angle = 1; //flat threshhold angle
const int flat_samples = 5; //threshhold for # of samples to be considered on flat surface
float Angle = 0; //integrated angle
...
void check_ramp(){  //checks if reached bottom of end ramp
  float a = Angle;

  switch(ramp_stage){

  case 0: //before enter ramp
    if (fabs(a) > ramp_angle) ramp_stage = 1;
    break;

  case 1: //in maze
    if (fabs(a) < flat_angle){
      flat_count++;
      if (flat_count >= flat_samples){  //assume on flat if true
        Angle = 0;
        ramp_stage = 2;
        flat_count = 0;
      }
  ...
  case 2: //checking for exit ramp
    if (fabs(a) > ramp_angle){
      ramp_stage = 3;
      flat_count = 0;
    }
    break;

  case 3: //on exit ramp, cheecking for floor
    if (fabs(a) < flat_angle){
      flat_count++;
      if (flat_count >= flat_samples){
        motor_off2();
        done = true;
      }
    }
...
void angle_update(){
  float wx = mpu6050_read_gyro_x_dps();  // deg/s
  Angle += wx * 0.01;

  Serial.print("wx = ");                          //delete*******
Serial.print(wx);
Serial.print("   angle = ");
Serial.println(Angle);
}

void CELEBRATE(){   //yaay maze complete
  while(true){
    led_off();
    _delay_ms(100);
    led_on();
...
int main(){
  ...
  mpu6050_calibrate_gyro_z(200);    //initialize and calibrate gyros (about 1 sec each)
  mpu6050_calibrate_gyro_x(200);

  //stops for 3 seconds with gryo callibration before moving to maze
  ...
  while (LineFlag == true && done == false){   //navigate once in the maze
    ...
    angle_update();
    check_ramp();
    _delay_ms(10); //doesnt work without                **was adjusted**
  }
